<?php
// seed_demo_content.php — everything in the demo dataset except the workspace row.
//
// Included by seed_demo.php (CLI, after a wipe) and by api/engine/workspace_clone.php (HTTP, to fill
// a brand-new campaign). It expects $conn, $W (the workspace id) and the x()/xid()/xrows() helpers
// plus the small date helpers to exist already; the clone library defines shims for them.
if (!isset($conn) || !isset($W)) { fwrite(STDERR, "seed_demo_content.php needs \$conn and \$W
"); exit(1); }
...
// Public holidays: England and Wales for the two years the demo spans. Computed from the rules by
// api/holidays.php, not listed here, so the seed and the importer can never drift apart.
require_once __DIR__ . '/api/lib.php';
require_once __DIR__ . '/api/holidays_rules.php';
$N_HOL = 0;
foreach ([2026, 2027] as $yr) {
  foreach (uk_bank_holidays($yr, 'england-and-wales') as $h) {
    xid($conn, 'public_holidays', ['workspace_id' => $W, 'day' => $h['day'], 'label' => $h['label'],
      'region' => null, 'source' => 'import', 'created_by' => $U['admin'], 'created_at' => '2026-03-30 09:00:00']);
    $N_HOL++;
  }
}

// Annual leave entitlement: the workspace default is 25 days, with two people on something else.
x($conn, "UPDATE dbo.people SET annual_leave_days = 30 WHERE id = ?", [$PN['Priya']]);
x($conn, "UPDATE dbo.people SET annual_leave_days = 20 WHERE id = ?", [$PN['Hana']]);

// capacity_days is DERIVED, never hand-rolled: the seed calls the same function the nightly cycle
// does, so the demo cannot disagree with the engine about what a day is worth. It was written out
// by hand here once, and the copy drifted — it knew nothing about bank holidays.
require_once __DIR__ . '/api/engine/capacity.php';
$N_CAP = derive_capacity($conn, $W, '2026-08-31', '2027-03-05');

...
def prepare(model: dict) -> Prepared:
    ...
    # Capacity per (person, bucket) in quarter-days: (planned_capacity, reserve).
    ...
    for p in people:
        ...
        for b in buckets:
            avail = reserve = 0.0
            for day in b.days:
                pos = day_list.index(day) if day in day_list else -1
                if 0 <= pos < len(cap_pairs):
                    a, r = cap_pairs[pos]
                    avail += float(a)
                    reserve += float(r)
            planned_q = int(round(max(0.0, avail - reserve) / hours_per_day * QUARTER))
            reserve_q = int(round(max(0.0, reserve) / hours_per_day * QUARTER))
            prepared.capacity[(p["id"], b.index)] = (planned_q, reserve_q)

    for item in items:
        prepared.eligible[item["id"]] = _eligible_people(item, people)

    # Committed baseline spread evenly across the assignment's buckets.
    for a in model.get("committed") or []:
        frm, to = _d(a["from_date"]), _d(a["to_date"])
        span = [day for day in day_list if frm <= day <= to]
        if not span:
            continue
        per_day = float(a.get("allocation_pct", 100)) / 100.0 * QUARTER
        for day in span:
            b = bucket_of_day.get(day)
            if b is None:
                continue
            key = (a["work_item_id"], a["person_id"], b)
            prepared.committed[key] = prepared.committed.get(key, 0) + int(round(per_day))
    return prepared
...
def solve(model: dict, budget_seconds: float = 60.0) -> dict[str, Any]:
        ...
        for pid in people_ids:
            for b in prep.buckets:
                ...
                if earliest and b.end < earliest:
                    continue
                var = m.NewIntVar(0, min(ceiling, remaining_q), f"a_{iid}_{pid}_{b.index}")
    ...
    # Locks: everything inside the freeze horizon is reproduced exactly, as are
    # user-fixed assignments and (on an urgent cycle) everyone outside the scope.
    locked_people = {p["id"] for p in prep.people if scope and p["id"] not in scope}
    for ca in model.get("committed") or []:
        frm = _d(ca["from_date"])
        pinned = bool(ca.get("locked")) or ca["person_id"] in locked_people
        if freeze_end and frm <= freeze_end:
            pinned = True
        if not pinned:
            continue
        for (iid, pid, b), qty in prep.committed.items():
            if iid == ca["work_item_id"] and pid == ca["person_id"]:
                var = a.get((iid, pid, b))
                if var is not None:
                    m.Add(var == qty)
